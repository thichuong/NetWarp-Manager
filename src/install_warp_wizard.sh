...
echo -e "${CYAN}============================================================${NC}"
echo ""
echo -e "Trình hướng dẫn này giúp bạn cài đặt Cloudflare WARP an sau."
...
# Check and install prerequisites on Ubuntu/Debian
if [ "$OS_TYPE" = "debian" ]; then
    ...
    if [ ${#PREREQS[@]} -ne 0 ]; then
        echo -e "${YELLOW}[Chuẩn bị] Phát hiện thiếu các gói bổ trợ: ${PREREQS[*]}.${NC}"
        echo -e "Tiến hành cài đặt tự động qua APT..."
        echo -e "Lệnh: ${BLUE}sudo apt-get update && sudo apt-get install -y ${PREREQS[*]}${NC}"
        echo -n "Nhấn ENTER để bắt đầu cài đặt (hoặc Ctrl+C để hủy)... "
        read -r
        if sudo apt-get update && sudo apt-get install -y "${PREREQS[@]}"; then
            echo -e "${GREEN}-> Cài đặt các gói bổ trợ thành công!${NC}"
        else
            echo -e "${RED}-> Thất bại khi cài đặt gói bổ trợ! Có thể gặp lỗi khi chạy bước tiếp theo.${NC}"
            echo -n "Nhấn ENTER để tiếp tục hoặc Ctrl+C để thoát... "
            read -r
        fi
        echo ""
    fi
...
# Verify if Cloudflare repository supports the detected codename, else fallback to noble/bookworm
if [ "$OS_TYPE" = "debian" ] && [ -n "$CODENAME" ]; then
    HTTP_STATUS=$(curl -o /dev/null -s -w "%{http_code}" "https://pkg.cloudflareclient.com/dists/${CODENAME}/Release")
    if [ "$HTTP_STATUS" != "200" ]; then
        echo -e "${YELLOW}[Cảnh báo] Kho lưu trữ Cloudflare chưa hỗ trợ phiên bản '${CODENAME}' (HTTP ${HTTP_STATUS}).${NC}"
        # Determine fallback base distro (Ubuntu vs Debian)
        if [[ "$ID" == "ubuntu" || "$ID_LIKE" == *"ubuntu"* || "$ID" == "linuxmint" || "$ID" == "pop" ]]; then
            FALLBACK_CODENAME="noble"
            echo -e "Tự động chuyển hướng sử dụng cấu hình tương thích của Ubuntu ${GREEN}${FALLBACK_CODENAME}${NC}."
        else
            FALLBACK_CODENAME="bookworm"
            echo -e "Tự động chuyển hướng sử dụng cấu hình tương thích của Debian ${GREEN}${FALLBACK_CODENAME}${NC}."
        fi
